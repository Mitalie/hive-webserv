	...
	const RouteConfig& route)
{
	...
	if (!isMethodAllowed(header.method(), route.allowedMethods))
		return std::make_unique<ErrorRequestHandler>(manager, header, 405); // Method Not Allowed
	...
	if (isCgiRequest(cleanPath, route.cgiInterpreters)) {
		...
		if (!isPathWithinRoot(cgiPath, route.root))
			return std::make_unique<ErrorRequestHandler>(manager, header, 403); // Forbidden - path traversal attempt
		
		if (!std::filesystem::exists(cgiPath))
			return std::make_unique<ErrorRequestHandler>(manager, header, 404); // CGI script not found
	...
	if (std::filesystem::exists(path) && std::filesystem::is_directory(path)) {
		...
		if (!route.index.empty()) {
			...
			return std::make_unique<AutoindexRequestHandler>(manager, header, route);
				...
				return std::make_unique<ErrorRequestHandler>(manager, header, 403); // Forbidden
			}
		} else if (route.autoindex) {
	...
	if (!std::filesystem::exists(path)) {
		// File does not exist
		return std::make_unique<ErrorRequestHandler>(manager, header, 404); // Not Found
	}
	...
	return std::make_unique<FileRequestHandler>(manager, header, route);
}
...
std::unique_ptr<IRequestHandler> router(IRequestManager& manager, const Header& header, const ListenerConfig& config)
{
	...
	if (!serverPtr)
		return std::make_unique<ErrorRequestHandler>(manager, header, 500); // Internal Server Error - no config
	...
	for (const RouteConfig& route : server.routes) {
		...
		{
			return handleRequestForRoute(manager, header, route);
		}
	}
	// No matching route found: return 404 handler
	return std::make_unique<ErrorRequestHandler>(manager, header, 404);
}
